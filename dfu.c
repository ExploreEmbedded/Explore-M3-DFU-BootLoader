...
__attribute__ ((packed))
{
	...
	usbdesc_string_l(15) iManufacturer;
	usbdesc_string_l(10) iProduct;
	usbdesc_string_l(14) iInterface;
...
{
	{
		...
		0x1209,					// idVendor
		0x0001,					// idProduct
		0x0000,					// bcdDevice (serial number)
		1,							// iManufacturer
		2,							// iProduct
		0,							// iSerial
	...
	},
	{
    	...
    	usbstring(15, "ExploreEmbedded"),
	usbstring(10 , "Explore M3"),
	usbstring(14, "Explore M3 DFU"), 
...
// 	UART0_Printf("user flash: %p\n\r", flash_p);
}

void DFU_GetStatus(CONTROL_TRANSFER *control)
{
	UART0_Printf("DFU:GETSTATUS\n\r");
...
{
	UART0_Printf("DFU:GETSTATE\n\r");
	control->buffer = &current_state;
	control->bufferlen = 1;
}
volatile int dfuDownloadStarted=0;
void DFU_Download(CONTROL_TRANSFER *control)
{
	UART0_Printf("DFU:DNLOAD\n\r");
    dfuDownloadStarted = 1;
	...
	if ((control->setup.bmRequestType & 0x7F) == 0x21)
	{
		switch(control->setup.bRequest)
		{
			case DFU_GETSTATUS:
			{
				...
				UART0_Printf("new state is %d\n\r", current_state);

				if (current_state == dfuMANIFESTWAITRESET)
				{
					usb_disconnect();
					UART0_Printf("MANIFEST COMPLETE, usb disconnected\n\r");
				}
...
// 					}
					...
					{
						UART0_Printf("write flash error %d\n\r", r);
...
int DFU_DownloadStartStatus()
{
    return dfuDownloadStarted;
}
