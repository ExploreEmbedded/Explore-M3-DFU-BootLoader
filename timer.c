...
**************************************************************************************************/
volatile timerConfig_t TimerConfigTable[TIMER_MAX] = 
{/* userFunPtr              Timer Struct        IRQ Number       PCONP bit    pclkBit    */
   { NULL,    LPC_TIM0, TIMER0_IRQn,    SBIT_PCTIM0, PCLK_TIMER0, },
...
void TIMER_Start(uint8_t timerNumber_u8)
{
    if(timerNumber_u8<TIMER_MAX)
    {
        ...
        TimerConfigTable[timerNumber_u8].TIMx->TCR  = ((uint32_t)1<<SBIT_CNTEN); /* Start timer by setting the Counter Enable*/ 
        LPC_TIM0->TCR  = (1 <<SBIT_CNTEN);
        UART0_Printf("\n\rTimer Start %d",timerNumber_u8);  
    }
}
